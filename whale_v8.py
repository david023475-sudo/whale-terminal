...
def page_analysis(run_analysis: bool) -> None:
    ...
    with st.spinner(f"Fetching {ticker} market data…"):
        try:
            ...
            # ── Dynamic timeframe data pipeline ───────────────────────────────
            # _disp_hist  = what actually renders on the chart (respects user's
            #               timeframe selection: 1D/5D intraday, 1M-5Y daily/weekly)
            # _rsi_hist   = always 1-year daily, used ONLY for RSI/MACD warm-up
            #               (Wilder's EWM needs >=200 bars; intraday frames lack this)
            #
            # Timeframe to interval mapping (from RANGE_MAP):
            #   1D  5-min bars    5D  15-min bars
            #   1M-6M  daily      1Y-5Y  daily or weekly
            _tf_period   = RANGE_MAP[timeframe]["period"]
            _tf_interval = RANGE_MAP[timeframe]["interval"]

            # Display history: exactly what the user selected
            _disp_hist = get_stock_history(ticker, _tf_period, _tf_interval)
            if _disp_hist.empty:
                _disp_hist = hist   # ultimate fallback to sidebar-fetched hist

            # RSI/MACD warm-up: always 1-year daily regardless of timeframe
            # so indicator values are stable even on 1D/5D intraday views
            ...
            fig.update_layout(
                ...
                # Styled to match TradingView CrosshairMode.Normal:
                #   color  #758696 (muted slate, not glowing blue)
                #   style  dotted  (dash="dot")
                #   mode   "across" only — no axis label pin (= Normal, not Magnet)
                xaxis=dict(
                    **_AXIS_CFG,
            ...
            fig.update_yaxes(
                ...
                **_AXIS_CFG,
                tickprefix="$",
                side="right",
                showspikes=True, spikecolor="#758696", spikethickness=1,
            )
